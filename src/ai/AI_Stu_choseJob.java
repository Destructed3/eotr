...
public class AI_Stu_choseJob {
    ...
    private boolean jobAv(int topic) {
        try {
            return dsk.getRes().getLAJ().stream().anyMatch(pAJ -> pAJ.getTopic()==topic);
        }catch(NullPointerException ex) {
            System.out.println(dsk.getRes().getLAJ());
            System.out.println("-----");
            return false;
        }
    }
    ...
    private ActivityJob find_best_job(List<int[]> l) {
        ...
        while(!lPref.isEmpty()) {
            ...
            if(isJobAv(attr)) {
                return dsk.getRes().getLAJ().stream().filter(pAJ -> pAJ.isActive() && pAJ.getHostNr()==0 && pAJ.getTopic()==attr).findAny().get();
            } else {
    ...
    private boolean isJobAv(int attr) {
        return dsk.getRes().getLAJ().stream().anyMatch(pAJ -> pAJ.isActive() && pAJ.getTopic()==attr);
    }
